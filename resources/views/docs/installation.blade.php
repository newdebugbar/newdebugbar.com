...
>
    <x-docs.page-header category="Getting started" title="Install The New Debug Bar">
        Add it to your Laravel app with one Composer command. Laravel discovers the package and the bar appears
        automatically while you work locally.
    </x-docs.page-header>
    ...
    @if ($installation['prerelease'])
        ...
        >
            The New Debug Bar has not tagged version 1.0 yet, so the command below installs the current
            <code class="font-mono text-[0.9em]">{{ $installation['constraint'] }}</code> build.
        </x-docs.callout>
    @endif
    ...
    <x-docs.section id="install" title="Install the package">
        ...
        <p class="mt-4 text-sm leading-6 text-zinc-500 dark:text-zinc-500">
            ...
            <code class="font-mono text-[0.9em] text-zinc-700 dark:text-zinc-300">require-dev</code> so production
            installs skip it.
        </p>
    </x-docs.section>
    ...
    <x-docs.section id="requirements" title="Requirements">
        ...
        <ul class="mt-5 space-y-3" role="list">
            <x-docs.check-item> PHP 8.1 or newer. </x-docs.check-item>
            <x-docs.check-item> Laravel 10 or newer. </x-docs.check-item>
            <x-docs.check-item>
                A Laravel app running in its
        ...
        <x-docs.callout class="mt-6" title="Livewire compatibility:">
            The New Debug Bar uses Livewire 4 for its own interface. Your app does not need to use Livewire, but apps
            that already use Livewire 3 are not supported.
        </x-docs.callout>
    </x-docs.section>

    <x-docs.section id="verify" title="Check that it works">
        <p class="mt-4 text-base leading-7 text-zinc-600 dark:text-zinc-400">
            Open any normal page in your app while Laravel is using the
            <code class="font-mono text-[0.9em] text-zinc-950 dark:text-zinc-100">local</code> environment. The bar
            should appear at the bottom of the browser.
        </p>
        ...
        <x-docs.figure
            class="mt-7"
            caption="Open the compact bar to inspect the request, query count, duration, and other captured details."
        >
            <x-screenshots.request-inspector
                alt="The New Debug Bar open over a Laravel page with request, query, and duration details"
                loading="lazy"
            />
        </x-docs.figure>
    </x-docs.section>
    ...
    <x-docs.section id="configuration" title="Optional configuration">
        ...
        <p class="mt-6 text-base leading-7 text-zinc-600 dark:text-zinc-400">
            To leave the package installed but turn it off, add this to your
            <code class="font-mono text-[0.9em] text-zinc-950 dark:text-zinc-100">.env</code> file:
        </p>
    ...
    <x-docs.section id="troubleshooting" title="Troubleshooting">
        <div class="mt-5 divide-y divide-zinc-200 border-y border-zinc-200 dark:divide-white/10 dark:border-white/10">
            <x-docs.disclosure summary="The bar does not appear">
                Confirm that Laravel reports the
                <code class="font-mono text-[0.9em] text-zinc-900 dark:text-zinc-200">local</code> environment and that
                <code class="font-mono text-[0.9em] text-zinc-900 dark:text-zinc-200">NEWDEBUGBAR_ENABLED</code> is not
                set to <code class="font-mono text-[0.9em] text-zinc-900 dark:text-zinc-200">false</code>. If your app
                uses another name for local work, publish the config and add that name to
                <code class="font-mono text-[0.9em] text-zinc-900 dark:text-zinc-200">newdebugbar.environments</code>.
                After changing config, run
                <code class="font-mono text-[0.9em] text-zinc-900 dark:text-zinc-200">php artisan config:clear</code>
                and refresh the page.
            </x-docs.disclosure>
            <x-docs.disclosure summary="Composer reports a Livewire conflict">
                <p>
                    The New Debug Bar requires Livewire 4. Apps that already depend on Livewire 3 are not supported yet,
                    even though an app does not otherwise need to use Livewire itself. Ask Composer which dependency
                    blocks Livewire 4:
                </p>
