...
<x-layouts.docs
    ...
    og-description="Turn the exact profile ID returned by The New Debug Bar on a test response into focused performance and correctness assertions."
    page-title="Testing"
    :sections="[
...
>
    <x-docs.page-header
        category="Reference"
        title="Protect important request profiles with Pest"
    >
        Capture the real Laravel response, read its exact profile ID, and assert
        only the error or performance limits that represent stable product
        behavior.
    </x-docs.page-header>

    <x-docs.section
        id="enable"
        title="Enable profiling before the test app boots"
    >
        ...
        <x-docs.callout
            class="mt-6"
            title="Enable it only for suites that use profiles:"
        >
            profiling adds collection work and may inject the local bar into
            eligible HTML responses. Keep ordinary response tests separate when
            they do not need request-profile evidence.
        </x-docs.callout>
    </x-docs.section>

    <x-docs.section
        id="profile-id"
        title="Read the exact profile ID from the response"
    >
    ...
    <x-docs.section id="assertions" title="Use focused profile assertions">
        <div
            class="mt-6 overflow-x-auto rounded-xl border border-zinc-200 dark:border-white/10"
        >
            <table class="w-full min-w-[44rem] text-left text-sm">
                <thead
                    class="bg-zinc-50 text-zinc-950 dark:bg-white/[0.035] dark:text-white"
                >
                    <tr>
                ...
                <tbody
                    class="divide-y divide-zinc-200 text-zinc-600 dark:divide-white/10 dark:text-zinc-400"
                >
                    <tr>
                        <td class="px-4 py-3">
                            <code class="font-mono text-[0.9em]"
                                >assertNoErrors()</code
                            >
                        </td>
                        <td class="px-4 py-3">
                            No error response or exception finding
                        </td>
                    </tr>
                    <tr>
                        <td class="px-4 py-3">
                            <code class="font-mono text-[0.9em]"
                                >assertNoRepeatedQueries()</code
                            >
                        </td>
                        <td class="px-4 py-3">
                            No repeated normalized query pattern
                        </td>
                    </tr>
                    <tr>
                        <td class="px-4 py-3">
                            <code class="font-mono text-[0.9em]"
                                >assertNoLikelyNPlusOneQueries()</code
                            >
                        </td>
                        <td class="px-4 py-3">No likely N+1 finding</td>
                    </tr>
                    <tr>
                        <td class="px-4 py-3">
                            <code class="font-mono text-[0.9em]"
                                >assertQueryCountAtMost($count)</code
                            >
                        </td>
                        <td class="px-4 py-3">Maximum query count</td>
                    </tr>
                    <tr>
                        <td class="px-4 py-3">
                            <code class="font-mono text-[0.9em]"
                                >assertQueryTimeAtMost($milliseconds)</code
                            >
                        </td>
                        <td class="px-4 py-3">Maximum total query time</td>
                    </tr>
                    <tr>
                        <td class="px-4 py-3">
                            <code class="font-mono text-[0.9em]"
                                >assertDurationAtMost($milliseconds)</code
                            >
                        </td>
                        <td class="px-4 py-3">Maximum full request duration</td>
                    </tr>
                    <tr>
                        <td class="px-4 py-3">
                            <code class="font-mono text-[0.9em]"
                                >assertPeakMemoryAtMost($megabytes)</code
                            >
                        </td>
                        <td class="px-4 py-3">Maximum peak memory</td>
                    </tr>
                </tbody>
            </table>
        </div>
    ...
    <x-docs.section
        id="budgets"
        title="Choose budgets that describe stable behavior"
    >
        <ul class="mt-5 space-y-3" role="list">
            <x-docs.check-item>
                Prefer query counts and error findings when they are stable
                across machines.</x-docs.check-item
            >
            <x-docs.check-item>
                Give timing and memory budgets room for normal CI
                variation.</x-docs.check-item
            >
            <x-docs.check-item>
                Use realistic database records so a likely N+1 pattern can
                actually appear.</x-docs.check-item
            >
            <x-docs.check-item>
                Assert the response behavior separately; a healthy profile does
                not replace feature assertions.</x-docs.check-item
            >
        </ul>
    </x-docs.section>
