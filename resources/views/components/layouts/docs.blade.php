    ...
    <main class="border-b border-zinc-950/[0.07] bg-white dark:border-white/[0.08] dark:bg-[#09090c]" data-docs-shell>
        <div class="mx-auto grid max-w-[76rem] lg:grid-cols-[14rem_minmax(0,1fr)] xl:grid-cols-[14rem_minmax(0,48rem)_14rem]">
            ...
            <article class="min-w-0 px-5 py-8 sm:px-8 sm:py-12 lg:px-10 lg:py-14 xl:px-12 xl:py-16">
                ...
                >
                    <a
                        class="rounded-sm underline decoration-zinc-300 underline-offset-4 hover:text-zinc-800 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-violet-500 dark:decoration-zinc-700 dark:hover:text-zinc-200"
                        href="{{ url('/') }}"
                    >Home</a>
            ...
            <aside
                class="hidden border-l border-zinc-950/[0.07] px-6 py-16 xl:block dark:border-white/[0.08]"
                @if ($sections !== []) aria-label="On this page" @else aria-hidden="true" @endif
            >
                @if ($sections !== [])
                    <nav class="sticky top-8">
                        <p class="text-xs font-semibold tracking-[0.16em] text-zinc-400 uppercase dark:text-zinc-500">
                            On this page
                        </p>
                        <ul class="mt-4 space-y-1 text-sm" role="list">
                            @foreach ($sections as $section)
                                <li>
                                    <a
                                        class="flex min-h-9 items-center rounded-lg px-2 text-zinc-600 transition-colors hover:bg-zinc-100 hover:text-zinc-950 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-violet-500 dark:text-zinc-400 dark:hover:bg-white/[0.06] dark:hover:text-white"
                                        href="#{{ $section['id'] }}"
                                    >
                                        {{ $section['label'] }}
                                    </a>
