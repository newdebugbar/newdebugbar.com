...
it('serves a public page through its intended view', function (string $uri, string $view) {
    ...
    'inspectors page' => ['/docs/inspectors', 'docs.inspectors'],
...
it('renders the social preview from a fixed local capture page', function () {
    ...
    expect($canvas)->toHaveCount(1)
        ...
        ->and($productScreenshot->item(0)?->getAttribute('data-request-inspector-view'))->toBe('requests')
        ->and($injectedToolbar)->toHaveCount(0);
});

it('publishes every public page through the XML sitemap', function () {
    ...
    get('/sitemap.xml')
        ...
        ->assertViewHas('urls', function (array $urls) use ($publicRoutes): bool {
            return in_array(url('/'), $urls, true) && $publicRoutes->every(
